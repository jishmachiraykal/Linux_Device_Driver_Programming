...
loff_t pcd_lseek (struct file *filp, loff_t offset, int whence){
    ...
    pr_info("Current value of the position = %lld\n", filp->f_pos);
...
ssize_t pcd_write (struct file *filp, const char __user *buff, size_t count, loff_t *f_pos){
    ...
    if(!count){
        pr_err("No space left on the devic\n");
        return ENOMEM;
    }    
    /* Copy from user */
...
static int __init pcd_driver_init(void)
{
    ...
    ret = cdev_add(&pcd_cdev,device_number,1); // registering one device
    if(ret < 0){
    ...
    if(IS_ERR(class_pcd)){
        ...
        ret = PTR_ERR(class_pcd); //PTR_ERR converts pointer to error code(int) and ERR_PTR converts error code(int) to pointer
        goto cdev_del; 
    }
